...
class xgbFeatureGenerator():
    ...
    def generateFeatureOfResultSet(self, index, textID, text, resultSet):
        ...
        for (theme, kywrd) in resultSet:
            ...
            resultFeature = np.concatenate((textFeature['embedding'], textFeature['tagProb'], themeFeature['embedding'], themeFeature['pos'], kywrdFeature['embedding'], kywrdFeature['pos']), axis=0)
...
def candidateRefine(wordList):
    ...
    for w1 in wordList:
        for w2 in wordList:
            if w1 != w2:
                if util.lcs_str(w1, w2) == w2 and w2 in w1:
                    refineDict[w1].append(w2)
    _tmp = []
    for i in refineDict.values():
        _tmp.extend(i)
    tmp = [word for word in set(_tmp)]
    resultList = [word for word in wordList if word not in tmp]
    return resultList
...
def addPairProb(df, data):
    with open('./xgb-data-%s.pkl' % data, 'rb') as fr:
        featureData = pickle.load(fr)
    contentList = df['sub_sents_tokenized'].values

    with open('./data/train_test_pair_match_score_new.pkl', 'rb') as fr:
        pairProbDF = pickle.load(fr)
        _ = pickle.load(fr)

    pairProbList = pairProbDF['score'].values
    ...
    for index, textList in enumerate(contentList):
        ...
        NEW_FEATURE = []
        for textID, text in enumerate(textList):
            tmp = np.asarray([pairProb for pairProb in pairProbList[index][textID]])
            NEW_FEATURE.append(tmp)
        try:
            NEW_FEATURE = np.vstack(NEW_FEATURE)
            X, y = featureData[index]
            ...
            print '{} is null'.format(index)
    with open('./xgb-data-%s-addPairProb.pkl' % data, 'wb') as fw:
        pickle.dump(featureData, fw)
    print '** Finished adding %s NEW Feature.' % data
...
def xgbSatcking(foldNum, k_fold=5):
    i = foldNum
    data = 'semiTrain-BIEonly-addTSN-addSubstringTag-addResultCount'

    with open('./xgb-data-%s.pkl' % data, 'rb') as fr:
        featureData = pickle.load(fr)

    np.random.seed(10)
    shuffleIndex = np.random.permutation(range(len(featureData)))
    featureData_shuffle = [featureData[index] for index in shuffleIndex]

    fold_size = len(featureData) / k_fold

    if i == 0:
        trainFeatureData = featureData_shuffle[fold_size:]
        validFeatureData = featureData_shuffle[:fold_size]
    if i <= k_fold - 2:
        startIndex = i * fold_size
        endIndex = (i + 1) * fold_size
        trainFeatureData = np.concatenate((featureData_shuffle[:startIndex], featureData_shuffle[endIndex:]))
        validFeatureData = featureData_shuffle[startIndex: endIndex]
    if i == k_fold - 1:
        startIndex = i * fold_size
        trainFeatureData = featureData_shuffle[:startIndex]
        validFeatureData = featureData_shuffle[startIndex:]

    X_train = []
    y_train = []
    for featureData in trainFeatureData:
        X_train.extend(featureData[0])
        y_train.extend(featureData[1])
    X_train = np.asarray(X_train)
    y_train = np.asarray(y_train)
    ...
    X_valid = []
    y_valid = []
    for featureData in validFeatureData:
        X_valid.extend(featureData[0])
        y_valid.extend(featureData[1])
    X_valid = np.asarray(X_valid)
    y_valid = np.asarray(y_valid)
    ...
    del featureData, trainFeatureData, validFeatureData, X_train, y_train, X_valid
    ...
    model = xgb.train(plst, xgb_train, num_rounds, watchlist, early_stopping_rounds=1000)
    model.save_model('./xgb-%s.model' % (data+'-'+str(i)))  # 用于存储训练出的模型
    print "best best_ntree_limit", model.best_ntree_limit
    cost_time = time.time() - start_time
    print "xgboost success!", '\n', "cost time:", cost_time, "(s)......"

    model = xgb.Booster({'nthread': -1})  # init model
    model.load_model('./xgb-%s.model' % (data+'-'+str(i)))  # load data
    y_pred = model.predict(xgb_valid)
...
def xgbPrediction(k_fold=5):
    ...
    for i in range(k_fold):
        ...
        model.load_model('./dic&corp/xgb-semiTrain-%s.model' % (data+'-'+str(i)))  # load data
...
if __name__ == '__main__':
    ...
    addNewTextTagProb(semiTrainDF, 'semiTrain-BIEonly')
    addSubstringTag(semiTrainDF, 'semiTrain-BIEonly-addTSN')
    addResultCount(semiTrainDF, 'semiTrain-BIEonly-addTSN-addSubstringTag')
    # Step3 - Add New Feature

    # Step4 - Train XGB
    xgbSatcking(foldNum=1, k_fold=5)
    # Step4 - Train XGB

    # Step5 - Semi Test Data Feature Extraction
    df = util.getSemiTestDF()
    themeTruthList = [theme for (theme, count) in util.readWordList('./data/themeTruthList-semiTrain.txt')]
    kywrdTruthList = [kywrd for (kywrd, count) in util.readWordList('./data/kywrdTruthList-semiTrain.txt')]

    contentList = semiTestDF['sub_sents_tokenized'].values
    textTagProb = semiTestDF['bie_prob_word'].values
    allTextList = getTextList(preTrainDF, preTestDF, semiTrainDF, semiTestDF)
    xFG = xgbFeatureGenerator(allTextList, textTagProb)
    print 'contentList len={}'.format(len(contentList))
    pool = Pool(processes=4)
    featureData = pool.map(multiprocessFeatureDataForTest, enumerate(contentList))
    with open('./xgb-data-semiTest-BIEonly.pkl', 'wb') as fw:
        pickle.dump(featureData, fw)
    print '** Finished saving the xgb-data-semiTest-BIEonly.'
    # Step5 - Semi Test Data Feature Extraction

    # Step6 - Add New Feature
    addNewTextTagProbForTest(semiTestDF, 'semiTest-BIEonly')
    addSubstringTagForTest(semiTestDF, 'semiTest-BIEonly-addTSN')
    addResultCountForTest(semiTestDF, 'semiTest-BIEonly-addTSN-addSubstringTag')
    # Step6 - Add New Feature

    # Step7 - Test Data Prediction
    xgbPrediction(k_fold=5)
    # Step7 - Test Data Prediction